...
describe('resource cloudScheduler', () => {
  test('updatePortfolioRoutine', async () => {
    const responsePromise = cadenza.webhook.cloudScheduler.updatePortfolioRoutine();
  ...
  test('updatePortfolioRoutine: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      cadenza.webhook.cloudScheduler.updatePortfolioRoutine({ path: '/_stainless_unknown_path' }),
